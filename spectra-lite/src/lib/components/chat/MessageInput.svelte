...
{#if loaded}
	<div class="w-full font-primary">
		...
		<div class="{transparentBackground ? 'bg-transparent' : 'bg-[#f5f7fa] dark:bg-[#1a1e2e] '} ">
			<div
							...
							>
								...
								<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
												...
												{#each toggleFilters as filter, filterIdx (filter.id)}
													<Tooltip content={filter?.description} placement="top">
														<button
															...
															type="button"
															class="font-secondary px-2 @xl:px-2.5 py-2 flex gap-1.5 items-center text-sm rounded-full transition-colors duration-300 focus:outline-hidden max-w-full overflow-hidden hover:bg-gray-50 dark:hover:bg-gray-800 {selectedFilterIds.includes(
																filter.id
															)
									...
									<div class="self-end flex space-x-1 mr-1 shrink-0">
										...
										{#if (taskIds && taskIds.length > 0) || (history.currentId && history.messages[history.currentId]?.done != true)}
											<div class=" flex items-center">
												<Tooltip content={$i18n.t('Stop')}>
													<button
														class="font-secondary bg-white hover:bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-800 transition rounded-full p-1.5"
														on:click={() => {
															stopResponse();
														}}
													>
										...
										<!-- DISABLED: Headset button (voice mode/call) -->
										...
										<button
											...
											transition rounded-full p-2 self-center shadow-sm hover:shadow-md"
											type="submit"
											disabled={prompt === '' && files.length === 0}
										>
											<Send class="w-5 h-5" strokeWidth="2.5" />
										</button>
									</Tooltip>
																					</div>
										{/if}
