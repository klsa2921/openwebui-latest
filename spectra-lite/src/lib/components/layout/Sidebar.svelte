...
<div
	...
	class="h-screen max-h-[100dvh] min-h-screen select-none 
		...
		shrink-0 text-sm fixed z-50 top-0 left-0 overflow-x-hidden 
...
>
	...
	>
		...
		{#if ($config?.features?.enable_workspace ?? true) && ($user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools)}
			<div class="px-1.5 flex justify-center text-gray-800 dark:text-gray-200">
				<a
					class="grow flex items-center space-x-3 rounded-lg px-2 py-[7px] hover:bg-gray-100 dark:hover:bg-gray-900 transition"
					href="/workspace"
